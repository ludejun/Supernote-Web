...
export const removeFirstNotes = () => {
  Storage.remove('firstNotes'); // 一个月的缓存有效时间
};
...
export const updateFirstNotes = (note: Note) => {
  ...
  const findIdList = (list, parentId, noteId) => {
    if (parentId === null || parentId === undefined) {
      ...
      for (let i = 0; i < list.length; i++) {
        if (list[i].noteId === parentId && list[i].childNodes && list[i].childNodes.length > 0) {
          ...
          list[i].childNodes && findIdList(list[i].childNodes, parentId, noteId);
        }
      }
    }
  };
